...
//    (https://github.com/stemkoski/stemkoski.github.com/blob/master/window.game.THREE.js/js/ParticleEngine.js).
//
...
SPE.utils = {
     ...
     * a new window.game.THREE.Vector3 instance with randomised values.
     *
     * @private
     *
     * @param  {window.game.THREE.Vector3} base
     * @param  {window.game.THREE.Vector3} spread
     * @return {window.game.THREE.Vector3}
     */
    randomVector3: function(base, spread) {
        var v = new window.game.THREE.Vector3();
    ...
    /**
     * Create a new window.game.THREE.Color instance and given a base vector and
     * spread range vector, assign random values.
     *
     * Note that window.game.THREE.Color RGB values are in the range of 0 - 1, not 0 - 255.
     *
     * @private
     *
     * @param  {window.game.THREE.Vector3} base
     * @param  {window.game.THREE.Vector3} spread
     * @return {window.game.THREE.Color}
     */
    randomColor: function(base, spread) {
        var v = new window.game.THREE.Color();
    ...
    /**
     * Create a new window.game.THREE.Vector3 instance and project it onto a random point
     * on a sphere with randomized radius.
     *
     * @param  {window.game.THREE.Vector3} base
     * @param  {Number} radius
     * @param  {window.game.THREE.Vector3} radiusSpread
     * @param  {window.game.THREE.Vector3} radiusScale
     *
     * @private
     *
     * @return {window.game.THREE.Vector3}
    ...
    randomVector3OnSphere: function(base, radius, radiusSpread, radiusScale, radiusSpreadClamp) {
        ...
        var vec = new window.game.THREE.Vector3(r * Math.cos(t), r * Math.sin(t), z);
    ...
    randomVector3OnDisk: function(base, radius, radiusSpread, radiusScale, radiusSpreadClamp) {
        ...
        var vec = new window.game.THREE.Vector3(Math.cos(t), Math.sin(t), 0).multiplyScalar(rand);
    ...
    /**
     * Create a new window.game.THREE.Vector3 instance, and given a sphere with center `base` and
     * point `position` on sphere, set direction away from sphere center with random magnitude.
     *
     * @param  {window.game.THREE.Vector3} base
     * @param  {window.game.THREE.Vector3} position
     * @param  {Number} speed
     * @param  {Number} speedSpread
     * @param  {window.game.THREE.Vector3} scale
     *
     * @private
     *
     * @return {window.game.THREE.Vector3}
    ...
    randomVelocityVector3OnSphere: function(base, position, speed, speedSpread, scale) {
        var direction = new window.game.THREE.Vector3().subVectors(base, position);
     ...
     * accordingly.
     *
     * @param  {window.game.THREE.Vector3} vector
     * @param  {window.game.THREE.Vector3} base
     * @param  {window.game.THREE.Vector3} spread
     *
     * @private
    ...
    /**
     * Randomize a window.game.THREE.Color instance and given a base vector and
     * spread range vector, assign random values.
     *
     * Note that window.game.THREE.Color RGB values are in the range of 0 - 1, not 0 - 255.
     *
     ...
     * @private
     *
     * @param  {window.game.THREE.Vector3} v
     * @param  {window.game.THREE.Vector3} base
     * @param  {Number} radius
     */
...
SPE.Group = function(options) {
    ...
    // Material properties
    that.blending = typeof options.blending === 'number' ? options.blending : window.game.THREE.AdditiveBlending;
    ...
    // vertices array.
    that.geometry = new window.game.THREE.Geometry();

    // Create the shader material using the properties we set above.
    that.material = new window.game.THREE.ShaderMaterial({
    ...
    // set so that window.game.THREE.js knows to update it on each frame.
    that.mesh = new window.game.THREE.ParticleSystem(that.geometry, that.material);
...
SPE.Group.prototype = {
     ...
     * that they need updating by window.game.THREE.js's internal tick functions.
     *
     * @private
     *
     * @return {this}
    ...
    _flagUpdate: function() {
        ...
        // ```ParticleSystem.sortParticles = true``` in window.game.THREE.r58 at least)
    ...
    addEmitter: function(emitter) {
        ...
        for (var i = start; i < end; ++i) {
            ...
            size[i] = new window.game.THREE.Vector3(
            ...
            angle[i] = new window.game.THREE.Vector4(
            ...
            opacity[i] = new window.game.THREE.Vector3(
     ...
     * @private
     *
     * @param  {window.game.THREE.Vector3} pos
     * @return {this}
     */
...
SPE.Emitter = function(options) {
    ...
    that.position = options.position instanceof window.game.THREE.Vector3 ? options.position : new window.game.THREE.Vector3();
    that.positionSpread = options.positionSpread instanceof window.game.THREE.Vector3 ? options.positionSpread : new window.game.THREE.Vector3();
    ...
    that.radiusScale = options.radiusScale instanceof window.game.THREE.Vector3 ? options.radiusScale : new window.game.THREE.Vector3(1, 1, 1);
    that.radiusSpreadClamp = typeof options.radiusSpreadClamp === 'number' ? options.radiusSpreadClamp : 0;

    that.acceleration = options.acceleration instanceof window.game.THREE.Vector3 ? options.acceleration : new window.game.THREE.Vector3();
    that.accelerationSpread = options.accelerationSpread instanceof window.game.THREE.Vector3 ? options.accelerationSpread : new window.game.THREE.Vector3();

    that.velocity = options.velocity instanceof window.game.THREE.Vector3 ? options.velocity : new window.game.THREE.Vector3();
    that.velocitySpread = options.velocitySpread instanceof window.game.THREE.Vector3 ? options.velocitySpread : new window.game.THREE.Vector3();
    ...
    // Colors
    that.colorStart = options.colorStart instanceof window.game.THREE.Color ? options.colorStart : new window.game.THREE.Color('white');
    that.colorStartSpread = options.colorStartSpread instanceof window.game.THREE.Vector3 ? options.colorStartSpread : new window.game.THREE.Vector3();

    that.colorEnd = options.colorEnd instanceof window.game.THREE.Color ? options.colorEnd : that.colorStart.clone();
    that.colorEndSpread = options.colorEndSpread instanceof window.game.THREE.Vector3 ? options.colorEndSpread : new window.game.THREE.Vector3();

    that.colorMiddle =
        options.colorMiddle instanceof window.game.THREE.Color ?
        options.colorMiddle :
        new window.game.THREE.Color().addColors(that.colorStart, that.colorEnd).multiplyScalar(0.5);
    that.colorMiddleSpread = options.colorMiddleSpread instanceof window.game.THREE.Vector3 ? options.colorMiddleSpread : new window.game.THREE.Vector3();
...
SPE.Emitter.prototype = {
     ...
     * @private
     *
     * @param  {window.game.THREE.Vector3} p
     */
