...
function Particle()
{
	this.position     = new window.game.THREE.Vector3();
	this.velocity     = new window.game.THREE.Vector3(); // units per second
	this.acceleration = new window.game.THREE.Vector3();
	...
	this.color   = new window.game.THREE.Color();
	this.opacity = 1.0;
...
Particle.prototype.update = function(dt)
{
	...
	if ( this.colorTween.times.length > 0 )
	{
		...
		this.color = new window.game.THREE.Color().setHSL( colorHSL.x, colorHSL.y, colorHSL.z );
	}
...
function ParticleEngine()
{
	...
	this.positionBase   = new window.game.THREE.Vector3();
	// cube shape data
	this.positionSpread = new window.game.THREE.Vector3();
	// sphere shape data
	this.positionRadius = 0; // distance from base at which particles start
	
	this.velocityStyle = Type.CUBE;	
	// cube movement data
	this.velocityBase       = new window.game.THREE.Vector3();
	this.velocitySpread     = new window.game.THREE.Vector3(); 
	// sphere movement data
	//   direction vector calculated using initial position
	this.speedBase   = 0;
	this.speedSpread = 0;
	
	this.accelerationBase   = new window.game.THREE.Vector3();
	this.accelerationSpread = new window.game.THREE.Vector3();	
	...
	// store colors in HSL format in a window.game.THREE.Vector3 object
	// http://en.wikipedia.org/wiki/HSL_and_HSV
	this.colorBase   = new window.game.THREE.Vector3(0.0, 1.0, 0.5); 
	this.colorSpread = new window.game.THREE.Vector3(0.0, 0.0, 0.0);
	...
	this.blendStyle = window.game.THREE.NormalBlending; // false;
	...
	//////////////
	// window.game.THREE.JS //
	//////////////
	
	this.particleGeometry = new window.game.THREE.Geometry();
	this.particleTexture  = null;
	this.particleMaterial = new window.game.THREE.ShaderMaterial( 
	{
		uniforms: 
		{
		...
		blending: window.game.THREE.NormalBlending, depthTest: true,
		
	});
	this.particleMesh = new window.game.THREE.Mesh();
}
...
ParticleEngine.prototype.setValues = function( parameters )
{
	...
	this.particleMesh = new window.game.THREE.ParticleSystem();
}
...
{
	var rand3 = new window.game.THREE.Vector3( Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5 );
	return new window.game.THREE.Vector3().addVectors( base, new window.game.THREE.Vector3().multiplyVectors( spread, rand3 ) );
}
...
ParticleEngine.prototype.createParticle = function()
{
	...
	if (this.positionStyle == Type.SPHERE)
	{
		...
		var vec3 = new window.game.THREE.Vector3( r * Math.cos(t), r * Math.sin(t), z );
		particle.position = new window.game.THREE.Vector3().addVectors( this.positionBase, vec3.multiplyScalar( this.positionRadius ) );
	}
	...
	{
		var direction = new window.game.THREE.Vector3().subVectors( particle.position, this.positionBase );
	...
	particle.color = new window.game.THREE.Color().setHSL( color.x, color.y, color.z );
...
ParticleEngine.prototype.initialize = function()
{
	...
	if ( this.blendStyle != window.game.THREE.NormalBlending) 
		this.particleMaterial.depthTest = false;
	
	this.particleMesh = new window.game.THREE.ParticleSystem( this.particleGeometry, this.particleMaterial );
	this.particleMesh.dynamic = true;
	this.particleMesh.sortParticles = true;
	window.game.scene.add( this.particleMesh );
}
